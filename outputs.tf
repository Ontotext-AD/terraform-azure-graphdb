...
output "dns_zone_name" {
  ...
  value       = var.deploy_external_dns_records ? module.external-dns-records[0].zone_name : null
}

output "dns_a_records" {
  description = "A records created in the DNS zone (if any)"
  value       = var.deploy_external_dns_records && length(module.external-dns-records[0].a_records) > 0 ? module.external-dns-records[0].a_records : null
}

output "dns_cname_records" {
  description = "CNAME records created in the DNS zone (if any)"
  value       = var.deploy_external_dns_records && length(module.external-dns-records[0].cname_records) > 0 ? module.external-dns-records[0].cname_records : null
}
