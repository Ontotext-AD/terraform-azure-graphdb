...
if [ "${node_count}" -eq 1 ]; then
  ...
  if [ "${disable_agw}" = "true" ]; then
  EXTERNAL_URL="https://${graphdb_external_address_fqdn}${context_path}/"
  else
...
EOF
  ...
  if [ "${disable_agw}" = "true" ]; then
  VHOSTS_VALUE="https://${graphdb_external_address_fqdn}${context_path}/,http://$${NODE_DNS}:7200"
  else
