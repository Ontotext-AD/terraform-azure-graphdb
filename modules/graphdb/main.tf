...
resource "azurerm_orchestrated_virtual_machine_scale_set" "graphdb" {
  name                = "vmss-${var.resource_name_prefix}"
  resource_group_name = var.resource_group_name
  location            = var.location

  platform_fault_domain_count = 1
  user_data_base64 = data.cloudinit_config.entrypoint.rendered


  # TODO figure out why ultra_ssd_disk options are not applied to PremiumV2_LRS
  data_disk {
    caching              = "None"
    disk_size_gb         = 50
    lun                  = 2
    storage_account_type = "PremiumV2_LRS"
#     ultra_ssd_disk_iops_read_write = 4000
#     ultra_ssd_disk_mbps_read_write = 150
  }
...
resource "azurerm_monitor_autoscale_setting" "graphdb_auto_scale_settings" {
  ...
  location            = var.location
  resource_group_name = var.resource_group_name
  target_resource_id  = azurerm_orchestrated_virtual_machine_scale_set.graphdb.id
