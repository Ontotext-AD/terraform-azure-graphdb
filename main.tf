...
locals {
  ...
  # Dynamically retrieve resource values after apply-time
  resources_to_lock = {
    ...
    "monitoring"                 = var.deploy_monitoring ? module.monitoring[0].la_workspace_id : ""
    "appconfig"                  = module.appconfig.app_configuration_id
    "application_gateway"        = var.disable_agw ? module.application_gateway[0].gateway_id : ""
    "vault"                      = module.vault[0].key_vault_id
    "vmss"                       = module.graphdb.vmss_resource_id
  }

  # Only keep non-null/empty values in resources_to_lock
