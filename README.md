...
| Version 2.x.x     | Version 11.0.x |
| Version 2.2.x     | Version 11.1.x |
...
| graphdb\_audit\_log\_role | Sets the audit role level. Accepted values (hierarchy, most to least verbose): ANY, USER, REPO\_MANAGER, ADMIN. See https://graphdb.ontotext.com/documentation/11.5/security-auditing.html                                                            | `string` | `"REPO_MANAGER"` | no |
...
```hcl
graphdb_properties_path = "path/to/graphdb.properties"
```

> **Note:** By default, any user with admin credentials can enable or disable security at will without reauthentication. However, security in GraphDB can also be set via the property graphdb.auth.security.enabled.
> When security is enabled or disabled this way, administrators will be unable to change the security configurations of GraphDB.
